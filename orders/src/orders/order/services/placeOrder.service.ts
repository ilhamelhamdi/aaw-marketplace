...
const getManyProductDatasByIdService = async (productIds: string[]) => {
    try {
        const response: AxiosResponse = await axios.post(`${process.env.PRODUCT_MS_URL}/api/product/many`, { productIds });
        return response;
    } catch (err: any) {
